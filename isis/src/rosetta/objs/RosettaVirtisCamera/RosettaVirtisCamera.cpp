...
//    PvlGroup &archive = lab.findGroup("Archive", Isis::Pvl::Traverse);
    ...
    m_exposureTime = toDouble(frameParam[0])/1000;
  ...
  void RosettaVirtisCamera::readHouseKeeping(const QString &filename,
   ...
   for (int i = 0; i < hktable.Records(); i++) {
     ...
     int scetWord1 = trec["dataSCET1"];
     int scetWord2 = trec["dataSCET2"];
     int scetWord3 = trec["dataSCET3"];
     long int seconds = scetWord1 * pow(2,16) + scetWord2;
     QString sclockString = toString(seconds) + ':' + toString(scetWord3);
     ...
     lineMidTime = getClockTime(sclockString, naifSpkCode()).Et();
