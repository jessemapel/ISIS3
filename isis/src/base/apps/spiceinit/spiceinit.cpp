...
namespace Isis {
  ...
  void spiceinit(UserInterface &ui, Pvl *appLog) {
    ...
    if (ui.GetBoolean("WEB")) {
      ...
      getUserEnteredKernel("LS", lk, ui);
      getUserEnteredKernel("PCK", pck, ui);
      getUserEnteredKernel("TSPK", targetSpk, ui);
      getUserEnteredKernel("FK", fk, ui);
      getUserEnteredKernel("IK", ik, ui);
      getUserEnteredKernel("SCLK", sclk, ui);
      getUserEnteredKernel("SPK", spk, ui);
      getUserEnteredKernel("IAK", iak, ui);
      getUserEnteredKernel("EXTRA", exk, ui);

      // Get shape kernel
      if (ui.GetString("SHAPE") == "USER") {
        getUserEnteredKernel("MODEL", dem, ui);
      }
   ...
   *               was entered.
   */
  void getUserEnteredKernel(const QString &param, Kernel &kernel, UserInterface &ui) {
